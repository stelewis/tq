...
The docs dependency audit uses `npm audit --package-lock-only` and only reruns in main CI when the Node or docs-toolchain surface changes. `package.json` is the sole owner of Node and npm versions: setup-node reads `engines.node` directly, the setup action validates `packageManager`, and `health doctor` consumes the same fields. The scheduled docs security workflow covers advisory churn for the VitePress toolchain between repository changes. The docs sync and docs build jobs follow the same model: they are skipped unless docs content, docs generator inputs, generated reference outputs, or docs-toolchain files changed.
