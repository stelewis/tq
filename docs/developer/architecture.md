...
- **Tooling adapters:** `crates/tq-docsgen` and `crates/tq-dev` own docs generation and the developer harness (repository policy, dependency maintenance, and release builds) so repository tooling follows the same Rust-first architecture as the runtime.
