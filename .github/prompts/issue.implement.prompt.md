...
Implement the GitHub issue specified by the user.

Fetch the issue, relevant code, and any references you need. Do not assume the issue text or existing code is correct. Build the right solution for the project, not the smallest change that preserves outdated structure.

## Large Issues

If the issue is too large for one pass, split it into smaller sub-tasks, create follow-up issues when useful, and report the remaining plan clearly.
